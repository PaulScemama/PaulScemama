...
$$
y = \text{sin}(2 \pi x) + \mathcal{N}(0, \beta^{-1}), \tag{1.1}
$$
...
Similar to the maximum likelihood section, once we've found the maximum a posteriori estimate for $\mathbf{w}$, which we will denote $\mathbf{w}_{\text{MAP}}$, we can use it to find the setting for the precision parameter $\beta$. We did not introduce a prior over $\beta$, so the negative logarithm of $(3.8)$ is

$$
-\text{ln}[p(\pmb{\mathsf{t}}|\pmb{\mathsf{x}}, \mathbf{w}, \beta) p(\mathbf{w}|\alpha)] = -[\text{ln}p(\pmb{\mathsf{t}}|\pmb{\mathsf{x}}, \mathbf{w}, \beta) + \text{ln} p(\mathbf{w}|\alpha)]. \tag{3.11}
$$
